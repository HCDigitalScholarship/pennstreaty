...
def break_texts_to_lines(texts, number_of_words_per_line):
    words=texts.split()
    words_count=len(words)
    print (words_count)
    big_loop_count=int(len(words)/number_of_words_per_line)+1
    ...
    while k <= big_loop_count:
        ...
        if k == big_loop_count:
            ...
            while i < number_of_words_per_line:
                line=line+words[i]+" "
                i+= 1
            print(line)
            lines.append(line)
...
class PageNumCanvas(canvas.Canvas):
        ...
        def draw_page_number(self,page_count):
                ...
                self.drawCentredString(4.25*inch,.25*inch+32,'Page %s of %s' % (str(self._pageNumber),str(page_count)))
...
def build(TEI_ID):
    ...
    y_index=8.2*inch
    for line in break_texts_to_lines(document.title,10):
        canv.drawCentredString(4.25*inch,y_index,line)
        y_index=7.95*inch
    ...
    for field in fields_in_display_order:#field (Authors) and Summary are special
        ...
        if field == "summary":
            try:
                info = metadata[field]
                summary_lines=break_texts_to_lines(metadata['summary'],10)
...
#the line breaks on the images
        ...
        transcription_text.setCharSpace(0)
        transcription_text.setWordSpace(2)
        text=soup.get_text()
